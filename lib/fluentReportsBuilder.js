...
class ReportRunnable
{
     ...
     * @private
     */
    _calculateOption(info, key, inOptions, callback) {
        ...
        if (typeof value.type !== 'undefined') {
            ...
            if (typeof info.data[value] === 'undefined' && typeof info.state.parentData[value] !== 'undefined') {
                ...
                if(inOptions.formatFunction){
                    let functionData;
                    for(let i=0;i<this._formatterFunctions.length;i++){
                        if(this._formatterFunctions[i].name === inOptions.formatFunction){
                            functionData = this._formatterFunctions[i];
                        }
                    }
                    if(functionData) {
                        functionData.class = functionData.class || "string";
                        if(functionData.class === "function" ){
                            functionData.function(info.data[inOptions[key]],info.data,(output)=>{
                                callback(key,output);
                            });
                            return;
                        }
                        else if(functionData.class === "string" ) {
                            if (!functionData.func) {
                                functionData.func = new Function('report', 'row', 'value', 'state', 'vars', 'done', functionData.function); // jshint ignore:line
                            }
                            functionData.func(info.report, info.data, info.data[key], info.state, info.variables, (output)=>{
                                callback(key,output);
                            });
                            return
                        }
    ...
    _handleOptions(inOptions, info, callback) {
        ...
        for (let key in inOptions) {
            if (inOptions.hasOwnProperty(key)) {
                counter++;
                this._calculateOption(info, key, inOptions, doneChecker);
            }
        }
        doneChecker();
    }
    ...
    _handlePrint(setting, info, callback) {
        ...
        if (setting.field) {
            if (!info.data || !info.data[setting.field] ) { output = ''; }
                ...
                if(setting.settings.formatFunction){
                    let functionData;
                    for(let i=0;i<this._formatterFunctions.length;i++){
                        if(this._formatterFunctions[i].name === setting.settings.formatFunction){
                            functionData = this._formatterFunctions[i];
                        }
                    }
                    if(functionData) {
                        functionData.class = functionData.class || "string";
                        if(functionData.class === "function" ){
                            functionData.function(info.data[setting.field],info.data,(output)=>{
                            ...
                            functionData.func(info.report, info.data, info.data[setting.field], info.state, info.variables, (output)=>{
                                this._handleOptions(setting.settings, info, (options) => {
                                    info.report.print(output, options, callback);
                                });
                            });
    ...
    constructor(ReportDesignLayout, reportData) {
        ...
        if (ReportDesignLayout) {
            if(ReportDesignLayout.formatterFunctions){
                this._formatterFunctions = ReportDesignLayout.formatterFunctions;
            }
