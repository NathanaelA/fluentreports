            ...
            if (typeof info.data[value] === 'undefined' && typeof info.state.parentData[value] !== 'undefined') {
                ...
                if(inOptions.formatFunction){
                    ...
                    if(!functionData && typeof inOptions.formatFunction === "number") {
                        functionData = formatterFunctions[inOptions.formatFunction - 2];
                    }
                    for(let i=0;i<formatterFunctions.length;i++){
                        if(formatterFunctions[i].name === inOptions.formatFunction){
                            functionData = formatterFunctions[i];
                        }
                    }
                    if(functionData) {
        ...
        if (setting.field) {
            if (!info.data || !info.data[setting.field] ) { output = ''; }
                ...
                if(setting.settings.formatFunction){
                    let functionData;
                    for(let i=0;i<formatterFunctions.length;i++){
                        if(formatterFunctions[i].name === setting.settings.formatFunction){
                            functionData = formatterFunctions[i];
                        }
                    }
                    if(functionData) {
    ...
    constructor(ReportDesignLayout, reportData) {
        ...
        if (ReportDesignLayout) {
            if(ReportDesignLayout.formatterFunctions){
                formatterFunctions = ReportDesignLayout.formatterFunctions;
            }
